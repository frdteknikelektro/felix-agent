...
Note: The workspace root is `$HOME` (`/home/node`). Runtime state (sessions, catalog, approvals, indexes, projects, tools) lives at the root level, not in a `workspace/` subdirectory.
...
Set `UID` / `GID` to match the host user that owns the workspace directory. On macOS and Windows Docker Desktop the defaults (1000:1000) usually work.
