...
`);
    ...
    it("dedicated-number reply survives a resolved JID suffix", async () => {
      cfg = await makeTestConfig("wa-wh-reply-jid-suffix-", {
        WHATSAPP_OWNER_JID: "owner@s.whatsapp.net",
        WHATSAPP_WEBHOOK_SECRET: "jid-suffix-test-secret",
      });
      await installFakeWacli(cfg, `
if [ "$1" = "doctor" ]; then
  printf '%s\\n' '{"data":{"linked_jid":"6281234567890@s.whatsapp.net","connected":true}}'
  exit 0
fi
if [ "$1" = "messages" ] && [ "$2" = "show" ]; then
...
`);
      const { engine: localEngine, ingest: localIngest } = makeMockEngine();
      const source = await startWhatsAppSource(cfg, localEngine);
