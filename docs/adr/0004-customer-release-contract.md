...
`0.1.1` is the first recommended production Felix Release. It is distributed as `frdinawan/felix-agent:0.1.1`, with Git tag `v0.1.1`; the same release build also publishes the `latest` convenience alias. The supported deployment environment is single-host Docker Compose on `linux/amd64`. Published `v0.1.0` artifacts remain immutable but are superseded.
...
Publication is the successful single AMD64 Docker build in the `Release`
