...
Human-created artifact and non-project descendant paths must use safe readable slugs (with lowercase file extensions), while Project descendants may retain ecosystem-required names. Every target must remain within the real path of its selected canonical category under `$WORKSPACE_DIR`, Session targets must match the active thread, symbolic links must not escape their category, existing regular files with multiple hard links are rejected conservatively, and no operation may introduce an undocumented workspace-root category. `WORKSPACE_FOLDER_STRUCTURE.md` is the detailed instruction contract; `AGENTS.md` and bundled filesystem-mutating Skills require applying it before each mutation.
