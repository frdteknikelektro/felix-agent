...
the same UUID in `id`. Use an atomic temp-file-and-rename write.
...
`detail`, or `silent`. A one-shot job becomes `completed` only after a successful run.
...
and success/failure status. Treat those records as the execution history.
