...
3. Resolve an intermediate WAV with `felix-workspace-path session-work "$FELIX_THREAD_DIR" "audio-transcription" "<filename>.wav"`; stop if rejected. Convert to that exact path with `ffmpeg -i <input-path> -ar 16000 -ac 1 "$WAV_PATH"`.
...
- Resolve each output before writing it: use `felix-workspace-path session-work "$FELIX_THREAD_DIR" "speech-synthesis" "<filename>.wav"` for intermediate audio and `felix-workspace-path session-attachment "$FELIX_THREAD_DIR" "<filename>.ogg"` for the delivered file. Pass exactly the returned path to Piper or ffmpeg; if the resolver rejects it, stop instead of hand-building a path.
