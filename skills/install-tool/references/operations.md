...
Enumerate the two bin directories, deduplicate command names, and inspect each through the check procedure:

```bash
find "$WORKSPACE_BIN" "$NPM_PREFIX/bin" -maxdepth 1 \( -type f -o -type l \) -print 2>/dev/null
```
