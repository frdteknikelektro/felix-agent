  ...
  async getTurnContext(input: { event: UniversalEvent }): Promise<SourceTurnContext> {
    ...
    return {
      behaviorInstructions: [
        ...
        "W5a. WhatsApp voice notes (PTT recordings) are audio attachments whose MIME type contains both `ogg` and `opus` (case-insensitive, e.g. `audio/ogg; codecs=opus`) — WhatsApp's in-app recorder always encodes them this way, which is how you tell a voice note apart from an uploaded audio file (see W5b). A voice note IS the user talking to you, so always transcribe it and treat the transcript as the user's message content before answering. First check duration: `ffprobe -v error -show_entries format=duration -of csv=p=0 <path>`; if it exceeds ~15 minutes, tell the user it's too long to auto-transcribe and ask for a shorter note instead — do not attempt the transcription. Otherwise: check for `whisper-cli` with `command -v whisper-cli`; if missing, use the install-tool skill once to install whisper.cpp (binary mode, official https://github.com/ggml-org/whisper.cpp releases) plus a small **multilingual** ggml model (`base` quantized `q5_1`, ~60MB — do NOT use a `.en`-suffixed model, those are English-only and will mis-transcribe or fail on other languages) — it persists in the workspace runtime so later turns reuse it without reinstalling. Convert to 16kHz mono WAV: `ffmpeg -i <path> -ar 16000 -ac 1 <path>.wav`. Then run `whisper-cli -m <model path> -f <path>.wav --no-timestamps` (whisper.cpp auto-detects the spoken language by default; do not force `--language en`) and use the printed transcript as the message content.",
        "W5b. Any other `audio/*` attachment (MIME does not contain both `ogg` and `opus` — e.g. mp3, m4a, wav, aac) is an uploaded/shared audio file, not speech directed at you — treat it like a document. Do NOT auto-transcribe it. First reply describing what you received: filename/local path, MIME type, and duration (same `ffprobe` command as W5a). Only run the transcription flow (same whisper-cli/ffmpeg steps and 15-minute duration guard as W5a) if the user's accompanying text explicitly asks about the audio's content (e.g. \"what does this say\", \"transcribe this\", \"summarize this recording\").",
