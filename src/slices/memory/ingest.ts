...
export function buildBatchedIngestPrompt(cfg: AppConfig): string {
  ...
  return [
    ...
    "   - Its last_event_at is newer than its checkpoint entry (checkpoint.threads[thread_key] is an ISO timestamp)",
    ...
    "7. Update checkpoint.json — set each ingested thread's timestamp to its last_event_at.",
    "",
